...
type ValidData = {
  document_title: string; document_type: string; document_number: string | null;
  signed_at: string; expires_at: string | null;
  signer_name: string; signer_nip: string | null; signer_position: string | null;
  document_hash: string; verification_token: string; verification_count: number;
  revoke_reason: string | null; revoked_at: string | null;
};
type Loaded =
  | { state: "loading" }
...
function Page() {
    ...
    (async () => {
      try {
        const r = await verify({ data: { token } }) as {
          valid: boolean;
          reason?: string;
          signed?: { document: { title: string; document_type: string }; signed_at: string; expires_at: string | null; document_hash: string; verification_token: string; verification_count: number; revoke_reason: string | null; revoked_at: string | null };
  ...
  async function reVerify() {
    ...
    try {
      ...
      if (r.match) {
        toast.success(`Dokumen COCOK. Ditandatangani ${new Date(r.signed_at).toLocaleString("id-ID")}`);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="container mx-auto flex-1 space-y-6 py-8">
        ...
        <Card className="mx-auto max-w-2xl">
          <CardHeader><CardTitle>Verifikasi Ulang dengan Upload PDF</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Input type="file" accept="application/pdf" onChange={(e) => setReUpload(e.target.files?.[0] ?? null)} />
            <Button onClick={reVerify} disabled={!reUpload || busy}>{busy ? "Memverifikasi…" : "Verifikasi Server-Side"}</Button>
            <p className="text-xs text-muted-foreground">PDF diunggah ke server. Server menghitung SHA-256 dan membandingkan dengan registry resmi (client hash tidak dipercaya).</p>
          </CardContent>
        </Card>
      </main>
      <Footer />
