...
type Sh = { id: string; nama: string; jam_masuk: string; jam_pulang: string; toleransi_menit: number; jenis: string; aktif: boolean };
...
function ShiftEditor({ list, onChange }: { list: Sh[]; onChange: () => void }) {
  const [form, setForm] = useState({ nama: "", jam_masuk: "08:00", jam_pulang: "16:00", toleransi_menit: 15, jenis: "pagi" as const });
  async function save() {
    try { await upsertShift({ data: { ...form, opd_id: null, aktif: true } }); toast.success("Shift disimpan"); onChange(); }
    catch (e) { toast.error((e as Error).message); }
  }
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-4">
          ...
          <input type="time" className="h-10 rounded-md border border-border bg-background px-3 text-sm" value={form.jam_masuk} onChange={(e) => setForm({ ...form, jam_masuk: e.target.value })} />
          <input type="time" className="h-10 rounded-md border border-border bg-background px-3 text-sm" value={form.jam_pulang} onChange={(e) => setForm({ ...form, jam_pulang: e.target.value })} />
          <input type="number" min={0} max={120} className="h-10 rounded-md border border-border bg-background px-3 text-sm" placeholder="Toleransi (menit)" value={form.toleransi_menit} onChange={(e) => setForm({ ...form, toleransi_menit: parseInt(e.target.value) || 0 })} />
        </div>
        <button onClick={save} className="mt-3 h-10 rounded-md bg-gradient-primary px-4 text-sm font-semibold text-primary-foreground">Simpan</button>
      </div>
      <div className="rounded-xl border border-border bg-card">
        ...
        {list.map((s) => (
          <div key={s.id} className="flex items-center justify-between border-b border-border px-4 py-3 last:border-0">
            <div><div className="font-semibold">{s.nama} <span className="text-xs text-muted-foreground">({s.jenis})</span></div><div className="text-xs text-muted-foreground">{s.jam_masuk}–{s.jam_pulang} · toleransi {s.toleransi_menit} mnt</div></div>
