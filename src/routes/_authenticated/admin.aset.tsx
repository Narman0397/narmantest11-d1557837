...
function AsetTab({ opds }: { opds: Opd[] }) {
  ...
  async function reload() { const r = await listAset({ data: {} }); setRows((r as unknown as { rows: AsetRow[] }).rows); }
...
function RiwayatTab() {
  ...
  async function load() { if (!asetId) return; const r = await listAsetRiwayat({ data: { aset_id: asetId } }); setRows((r as unknown as { rows: Riwayat[] }).rows); }
