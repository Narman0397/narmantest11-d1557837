...
// ===== Pemeliharaan =====
type PemRow = { id: string; aset_id: string; jadwal: string; jenis: string; status: string; biaya: number | null; vendor: string | null; aset: { kode: string; nama: string } | null };

function PemeliharaanTab() {
  const [rows, setRows] = useState<PemRow[]>([]);
  const [aset, setAset] = useState<Array<{ id: string; kode: string; nama: string }>>([]);
  const [busy, setBusy] = useState(false);
  const [form, setForm] = useState<{ aset_id: string; jadwal: string; jenis: string; biaya: string; vendor: string }>({ aset_id: "", jadwal: "", jenis: "", biaya: "", vendor: "" });
  ...
  async function save() {
    ...
    try {
      await upsertPemeliharaan({ data: {
        aset_id: form.aset_id, jadwal: form.jadwal, jenis: form.jenis,
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-5">
          ...
          <input type="date" value={form.jadwal} onChange={(e) => setForm({ ...form, jadwal: e.target.value })} className="rounded-md border border-border bg-background px-2 py-2 text-sm" />
