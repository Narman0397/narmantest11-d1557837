...
export function FieldEditor({
  ...
  const refField = field.visible_if?.field ? allFields.find((x) => x.kode === field.visible_if!.field) : null;
      ...
      <div className="mt-3 rounded-lg border border-dashed border-border">
        ...
        {openAdv && (
          <div className="space-y-3 border-t border-border bg-muted/30 p-3">
            {/* Validation rules */}
            <div>
              <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Aturan Validasi</div>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                {isText && (
                  <>
                    <NumInput label="Min karakter" value={v.minLength} onChange={(n) => patchValidation({ minLength: n ?? undefined })} disabled={readOnly} />
                    <NumInput label="Max karakter" value={v.maxLength} onChange={(n) => patchValidation({ maxLength: n ?? undefined })} disabled={readOnly} />
                    <div className="col-span-2">
                      <label className="text-[10px] uppercase text-muted-foreground">Pattern (regex)</label>
                      <input
                        value={v.pattern ?? ""}
                ...
                {isNumber && (
                  <>
                    <NumInput label="Min" value={v.min} onChange={(n) => patchValidation({ min: n ?? undefined })} disabled={readOnly} />
                    <NumInput label="Max" value={v.max} onChange={(n) => patchValidation({ max: n ?? undefined })} disabled={readOnly} />
                  </>
                )}
                {isFile && (
                  <>
                    <NumInput label="Max ukuran (MB)" value={v.maxSizeMb} onChange={(n) => patchValidation({ maxSizeMb: n ?? undefined })} disabled={readOnly} />
                    {field.tipe === "multi_file_upload" && (
                      <NumInput label="Max jumlah file" value={v.maxFiles} onChange={(n) => patchValidation({ maxFiles: n ?? undefined })} disabled={readOnly} />
                    )}
                    <div className="col-span-2">
                      <label className="text-[10px] uppercase text-muted-foreground">MIME / ekstensi (pisahkan koma)</label>
                      <input
                        value={(v.accept ?? []).join(", ")}
                        onChange={(e) => patchValidation({ accept: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })}
                        disabled={readOnly}
                ...
                {!isText && !isNumber && !isFile && (
                  <div className="col-span-4 text-xs text-muted-foreground">Tidak ada aturan tambahan untuk tipe ini.</div>
                )}
              </div>
            </div>
            ...
            {/* Conditional logic */}
            <div>
              <div className="mb-1 text-[10px] font-semibold uppercase text-muted-foreground">Logika Tampil (Conditional)</div>
              <div className="grid grid-cols-12 gap-2">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (!fld) {
                      ...
                      onChange({
                        ...field,
                        visible_if: { field: fld, op: field.visible_if?.op ?? "eq", value: field.visible_if?.value ?? "" },
                      });
                    }
                  }}
                  disabled={readOnly}
                ...
                >
                  ...
                  {otherFields.map((x) => (
                    <option key={x.kode} value={x.kode}>{x.kode} — {x.label}</option>
                  ))}
                </select>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    onChange({ ...field, visible_if: { ...field.visible_if, op: e.target.value as NonNullable<FormField["visible_if"]>["op"] } });
                  }}
                ...
                <div className="col-span-5">
                  {field.visible_if && !["filled", "empty"].includes(field.visible_if.op) ? (
                    refHasOptions && (field.visible_if.op === "eq" || field.visible_if.op === "neq") ? (
                      <select
                        value={typeof field.visible_if.value === "string" ? field.visible_if.value : ""}
                        onChange={(e) => onChange({ ...field, visible_if: { ...field.visible_if!, value: e.target.value } })}
                        disabled={readOnly}
                      ...
                      <input
                        value={Array.isArray(field.visible_if.value) ? field.visible_if.value.join(",") : (field.visible_if.value ?? "")}
                        onChange={(e) => {
                          const op = field.visible_if!.op;
                          const raw = e.target.value;
                          const val = op === "in" || op === "not_in" ? raw.split(",").map((s) => s.trim()).filter(Boolean) : raw;
                          onChange({ ...field, visible_if: { ...field.visible_if!, value: val } });
                        }}
                        disabled={readOnly}
                        placeholder={field.visible_if.op === "in" || field.visible_if.op === "not_in" ? "nilai1, nilai2" : "nilai"}
                        className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-xs disabled:opacity-60"
                      />
                    )
                  ) : (
              ...
              <p className="mt-1 text-[10px] text-muted-foreground">
                Field hanya muncul jika kondisi terpenuhi. Field tersembunyi tidak ikut divalidasi saat submit.
              </p>
            </div>
          </div>
        )}
