...
export async function stampSignature(pdfBytes: Uint8Array, p: StampPayload): Promise<Uint8Array> {
  ...
  pages.forEach((page, idx) => {
    ...
    page.drawLine({ start: { x: 30, y: footerY + 12 }, end: { x: width - 30, y: footerY + 12 }, thickness: 0.4, color: rgb(0.55, 0.55, 0.6) });
  ...
  const y = 50;
  last.drawRectangle({ x, y, width: panelW, height: panelH, borderColor: rgb(0.2, 0.2, 0.25), borderWidth: 0.8, color: rgb(0.98, 0.98, 1) });
  last.drawImage(qr, { x: x + 8, y: y + 18, width: 110, height: 110 });

  const tx = x + 124;
  let ty = y + panelH - 18;
  const line = (t: string, b = false, s = 9) => { last.drawText(t, { x: tx, y: ty, size: s, font: b ? bold : font, color: rgb(0.1, 0.1, 0.15) }); ty -= 11; };
  ...
  if (pages.length > 1) {
    ...
    const my = 50;
    first.drawRectangle({ x: mx - 4, y: my - 4, width: miniW + 8, height: miniW + 18, borderColor: rgb(0.2, 0.2, 0.25), borderWidth: 0.6, color: rgb(0.98, 0.98, 1) });
    first.drawImage(qr, { x: mx, y: my + 10, width: miniW, height: miniW });
    first.drawText("Verifikasi QR", { x: mx + 8, y: my, size: 6, font: bold, color: rgb(0.2, 0.2, 0.25) });
  }
...
export async function buildBasicPdf(input: {
  ...
  draw(input.opdNama.toUpperCase(), 50, y, 14, true); y -= 14;
  draw(`(${input.opdSingkatan})`, 50, y, 10); y -= 8;
  ...
  if (input.documentNumber) { draw(`Nomor: ${input.documentNumber}`, 50, y); y -= 20; }
  draw(input.title, 50, y, 13, true); y -= 22;
  ...
  if (input.pemohonNama) {
    draw(`Pemohon: ${input.pemohonNama}`, 50, y); y -= 12;
