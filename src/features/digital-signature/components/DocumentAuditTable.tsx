      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r.id}>
            ...
            <TableCell className="font-mono text-xs max-w-[260px] truncate">{JSON.stringify(r.metadata)}</TableCell>
          </TableRow>
        ))}
        {rows.length === 0 && (
          <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">Belum ada audit.</TableCell></TableRow>
        )}
      </TableBody>
    </Table>
  );
